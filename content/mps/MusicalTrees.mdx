...
2. Go through X number of generations **X is a parameter given
   - Evolve [selection + reproduction] (1 - 10 times) **you decide how many
   - Prune 
3. Final Prune

```
Every selection + reproduction cycle must start with the print statement
cout << "EVOLVE" << endl;

See the selection and reproduction phase details below for their print statements

For every generation, start by printing, 
...
- if `RP < SP(node)` --> select node
  ...
  - `SP(node)`: Selection probability of node: highest between the fitness score of the node divided by 100 or 0.10
...
For every node considered for selection (assuming your variable for the current node is called node)
cout << "node: ";
...
Each selected node creates and adds a new child to themselves using the `mutate` function to create the new motif for the child.
...
than 0.1 and greater than 0.5. If the new mutated value exceeds these, adjust it to the nearest valid value. For example, if the pitch 
...
- If the node that needs to be pruned has children, recursively prune all children and their children... etc. Afterward, if the node still has children, 
  rotate up one of its children into its place, combining their children.

The PruneNodes function is a required function and must implement the above steps for full points. 
The function does not need any print statements.

During each generation there will be a pruning phase.
``` 
Start by printing, 
cout << "PRUNE " << endl;
...
Then start the threhold for pruning at the value 10 and prune all nodes below that threshold. If there are still more than 
200 nodes in the tree, increase the threshold by 1 and prune again. 
``` 
After each prune print,
cout << "  prune cutoff: " << prune << endl;
cout << "  size: " << size_ << endl;
Then update the prune threshold.
```

For the final prune, you just need to print once, 
```
...
Finally, it will call GenerateMelody to create a single vector of all the notes in the tree. 
